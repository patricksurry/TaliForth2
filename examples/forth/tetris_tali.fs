...
\
\		Thank you.
\ https://rosettacode.org/wiki/Tetris/Ansi94_Forth
...
tetris >order           \ Add tetris wordlist to search order
tetris set-current      \ Compile new words to the tetris wordlist

decimal

\ Tali doesn't come with ON and OFF
: on    $FFFF swap ! ;
: off   $0000 swap ! ;

\ Tali doesn't come with >=
: >=   < invert ;
...
\ Tali doesn't have an MS word for millisecond delays
\ Adjust the busywork loop for your system.
\ This works on a 12 Mhz Planck SBC
\ : ms   0 do 200 0 do i drop loop loop ;
\ This works on the csim simulator running on a 2019 MB Air
: ms   0 do 1000 0 do i drop loop loop ;

\ Tali's S" compiles the string into the dictionary.
\ This code expects transient strings that use a buffer
\ not in the dictionary.  We change S" to " for non-dictionary
\ strings below when defining bricks using def-brick.

\ This version only allows one transient string at a time.
10 constant stringsize
create stringbuff stringsize allot
: " ( ccc" -- c-addr u ) ( Put a string into stringbuff )
   [char] " parse
   stringsize min ( No more than 80 chars )
   stringbuff ( Use the next available temporary string )
   swap 2dup 2>r cmove ( Copy into the current string )
   2r> ; ( c-addr u for result left on stack )

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ End of Tali specific stuff
...
\ Substitute Starting Forth's RNG 

variable seed

\ Tali doesn't have time&date
\ : randomize	time&date + + + + + seed ! ;

: randomize $AA55 seed ! ;

\ Use random from early ed of Starting Forth, p. 265
: rnd1 seed @ 31421 * 6927 + dup seed ! ;
\ returns 0..9 for u = 10
: random ( u1 -- u2 )
    rnd1 um* swap drop ;
