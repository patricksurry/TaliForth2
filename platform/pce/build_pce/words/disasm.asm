...
_is_rel:
                ...
                bpl +
                dec zpage+1,x             ; for negative offsets extend the sign bit so add works out
...
_printing_done:
                ...
                lda zpage+0,x                 ; All done?
                ora zpage+1,x
                beq _done

                lda zpage+1,x                 ; Catch mid-instruction ranges
                bmi _done
...
_no_prefix:
                ...
                ; ( addr u )
                lda zpage+2,x                     ; set payload addr for pictured literal
                sta tmp1                    ; just like a return address it points one
                lda zpage+3,x                     ; byte before the actual payload
                sta tmp1+1
