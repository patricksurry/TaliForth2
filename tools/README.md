...
Note: Many of these tools are no longer maintained.  But a few are, including:

- `forth_pack.py`: compact forth code to a single line, removing comments and collapsing excess whitespace to a single space
- `generate_glossary.py`: parse comments from `words/*.asm` into the Glossary of Forth Words in the documentation
- `generate_wordlist.py`: post-process 64tass assembler output to produce a table of word sizes for the documentation

Deprecated tools
---

- `reverse_asm.py`: reverse the order of words.  No longer maintained
- `code_statistics.py`: post-process Ophis(?) assembler output.  No longer maintained?


Code skeleton (deprecated)
---
...
tools require at least Python 3.5 - they will not run with Python 2.
    ...
    generate_assembler.py   - Create SAN assembler words
