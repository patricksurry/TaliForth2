...
image::pics/header_detail.png[]

[horizontal]
DC::
  *Disjoint code.* When DC=1, the word's code can be located
...
LC::
  *Long code.* The length of the word body can be recorded in
...
FP::
  *Far previous.* Normally we store a two byte `nt_prev` pointer
...
NOTE: We could also reorganize the built-in dictionary to save a little more memory
...
Finally we have the **name string** at offset between 4 and 8. The string is _not_
...
NOTE: Tali Forth 2 optimizes the implementation of `constant` (and `variable`, and `value`) to compile
