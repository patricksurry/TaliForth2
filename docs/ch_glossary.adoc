...
`-leading`:: _Tali String_ ( addr1 u1 -- addr2 u2 ) "Remove leading whitespace"
...
`.r`:: _ANS core ext_ ( n u -- ) "Print NOS as unsigned number with TOS width"
...
`;`:: _ANS core_ ( -- ) or ( -- xt ) for :noname "End compilation of new word"
...
`?do`:: _ANS core ext_ (C: -- ) ( limit start -- ) "Conditional loop start"
...
`at-xy`:: _ANS facility_ ( m n -- ) "Move cursor to position given"
...
`cmove>`:: _ANS string_ ( addr1 addr2 u -- ) "Copy bytes from high to low"
...
`do`:: _ANS core_ (C: -- ) ( limit start -- )  "Start a loop"
...
`evaluate*`:: _ANS  file_ ( addr u -- ) "Separately EVALUATE each line of a multi-line string"
...
`havekey`:: _Tali Forth_ ( -- addr ) "Return address of key? vector"
...
`int>name`:: _Tali Forth_ ( xt -- nt | 0 ) "Get name token from execution token"
...
`is`:: _ANS core ext_ ( xt "name" -- ) or (C: "name" ) "Set named word to execute xt"
...
`key?`:: _ANS core_ ( -- char ) "Return true if a character is available"
...
`literal`:: _ANS core_ ( n -- ) "Arrange for TOS to be pushed on stack at runtime"
...
`m*/`:: _ANS double_ ( d1 n1 n2 -- d2 ) "Multiply d1 by n1 and divide the triple result by n2.  All values are signed."
...
`sliteral`:: _ANS string_ (C: addr u -- ) ( -- addr u ) "Compile a string for runtime"
...
`split`:: _Gforth_ ( addr u c -- addr2 u2 addr u1 ) "Split string at c"
...
`strip-underflow`:: _Tali Forth_ ( -- addr ) "Return address where underflow flag is kept"
...
`unloop`:: _ANS core_ ( -- ) "Drop current loop control block"
...
`within`:: _ANS core ext_ ( n1 n2 n3 -- f ) "Test n1 within range [n2, n3) or outwith [n3, n2)"
