; BIND ( addr "<spaces>picture" -- xt | 0 )
; picture is a string like "XY-AP" showing how to bind register inputs and outputs to the data stack
; legal registers are AXYS with at least one hyphen separating inputs and outputs even if empty.
; Other valid pictures are "-A", "X-", "Y-P"
; Invalid pictures are "A - X" (no spaces), "A" (no hyphen), "AA-X" (no duplication), "AB-C" (only AXYP)

; new length is e93c - e85f = 221 bytes
;   : foo [ $1234 ] bind a-x ;
